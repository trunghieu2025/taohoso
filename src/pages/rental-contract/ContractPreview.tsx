  ...
  return (
    <>
      ...
      <p style={{ fontStyle: 'italic', fontSize: '0.82rem' }}>
        ...
        {today.getFullYear()}, tại {data.propertyAddress || '...............'},
        chúng tôi gồm có:
      </p>
      ...
      <p>
        CCCD/CMND số:{' '}
        <span className="contract-highlight">{data.landlordId || '...'}</span>
      ...
      {data.landlordBank && (
        <p>
          STK ngân hàng: {data.landlordBank} — {data.landlordBankName || ''}
        </p>
      )}
      ...
      <p>
        CCCD/CMND số:{' '}
        <span className="contract-highlight">{data.tenantId || '...'}</span>
      ...
      {data.propertyDescription && (
        <p>• Tình trạng: {data.propertyDescription}</p>
      )}
      {data.propertyEquipment && (
        <p>• Trang thiết bị: {data.propertyEquipment}</p>
      )}

      <h4>Điều 2: Giá thuê và phương thức thanh toán</h4>
      <p>
        • Giá thuê:{' '}
        <span className="contract-highlight">{formatVND(data.rentAmount)}</span>
        /tháng
        {data.rentAmountWords && <> ({data.rentAmountWords})</>}
      </p>
      <p>
        • Tiền đặt cọc: {formatVND(data.depositAmount)}
        {data.depositAmountWords && <> ({data.depositAmountWords})</>}
      </p>
      <p>
        • Thanh toán: {data.paymentMethod}, trước ngày {data.paymentDay || '05'}{' '}
        mỗi tháng
      </p>
      <p>
        • Tiền đặt cọc được hoàn trả khi Bên B trả nhà đúng hạn và không vi phạm
        hợp đồng
      </p>
      {(data.electricRate || data.waterRate) && (
        <>
          ...
          {data.electricRate && (
            <p>
              • Tiền điện: {Number(data.electricRate).toLocaleString('vi-VN')}{' '}
              VND/kWh
            </p>
          )}
          {data.waterRate && (
            <p>
              • Tiền nước: {Number(data.waterRate).toLocaleString('vi-VN')}{' '}
              VND/m³
            </p>
          )}
          {data.internetCost && (
            <p>
              • Internet: {Number(data.internetCost).toLocaleString('vi-VN')}{' '}
              VND/tháng
            </p>
          )}
      ...
      <p>
        • Từ ngày: {data.startDate || '...'} đến ngày: {data.endDate || '...'}
      </p>
      <p>
        • Hết hạn, nếu Bên B có nhu cầu tiếp tục thuê thì phải thông báo cho Bên
        A trước ít nhất 02 tháng
      </p>

      <h4>Điều 4: Quyền và nghĩa vụ của Bên A</h4>
      <p>4.1. Giao nhà cho Bên B đúng thời gian và tình trạng đã thỏa thuận</p>
      <p>
        4.2. Đảm bảo quyền sử dụng nhà hợp pháp, ổn định cho Bên B trong suốt
        thời hạn hợp đồng
      </p>
      <p>
        4.3. Bảo dưỡng, sửa chữa nhà định kỳ hoặc theo thỏa thuận (trừ hư hỏng
        do Bên B gây ra)
      </p>
      ...
      <p>
        4.7. Thực hiện nghĩa vụ tài chính với Nhà nước theo quy định pháp luật
      </p>
      ...
      <p>
        5.3. Giữ gìn, bảo quản nhà và trang thiết bị; bồi thường nếu làm hư hỏng
      </p>
      <p>
        5.4. Không được tự ý sửa chữa, cải tạo, thay đổi kết cấu khi chưa có sự
        đồng ý bằng văn bản của Bên A
      </p>
      <p>
        5.5. Không được cho thuê lại, chuyển nhượng HĐ khi chưa có sự đồng ý của
        Bên A
      </p>
      <p>
        5.6. Tuân thủ quy định pháp luật, nội quy khu dân cư, giữ gìn vệ sinh và
        trật tự
      </p>
      <p>
        5.7. Trả lại nhà khi hết hạn hoặc chấm dứt HĐ đúng tình trạng ban đầu
      ...
      <p>
        6.2. Một bên muốn chấm dứt trước hạn phải thông báo bằng văn bản trước
        ít nhất {data.noticePeriod || '30'} ngày
      </p>
      <p>
        6.3. Bên A được đơn phương chấm dứt nếu Bên B: không trả tiền thuê quá
        30 ngày, sử dụng sai mục đích, vi phạm pháp luật, hoặc gây thiệt hại
        nghiêm trọng
      </p>
      <p>6.4. Bên vi phạm phải bồi thường thiệt hại cho bên còn lại</p>
      <p>
        6.5. Khi chấm dứt HĐ, hai bên lập biên bản bàn giao nhà và thanh lý hợp
        đồng
      </p>
      ...
      <p>
        7.2. Mọi tranh chấp phát sinh được giải quyết thông qua thương lượng;
        nếu không thống nhất sẽ đưa ra Tòa án nhân dân có thẩm quyền
      </p>
      <p>
        7.3. Mọi sửa đổi, bổ sung phải được lập thành phụ lục hợp đồng bằng văn
        bản có chữ ký của hai bên
      </p>
      <p>
        7.4. Hợp đồng được lập thành 02 bản, mỗi bên giữ 01 bản, có giá trị pháp
        lý như nhau
      </p>
